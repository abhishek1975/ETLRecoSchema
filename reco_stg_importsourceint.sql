...
--
-- Table structure for table `stg_importsourceint`
--

DROP TABLE IF EXISTS `stg_importsourceint`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
 SET character_set_client = utf8mb4 ;
CREATE TABLE `stg_importsourceint` (
  `importSourceIntId` int(11) NOT NULL AUTO_INCREMENT,
  `jobId` int(11) NOT NULL,
  `jobImportId` int(11) NOT NULL,
  `organizationId` int(11) NOT NULL,
  `relationshipId` int(11) NOT NULL,
  `recordType` varchar(16) DEFAULT NULL COMMENT 'Either "Header", "Footer", "Transaction"',
  ...
  `amount_2` varchar(64) DEFAULT NULL,
  ...
  PRIMARY KEY (`importSourceIntId`),
  KEY `FK_STGIMPORTINT_JOBIMPORTS_idx` (`jobImportId`),
  KEY `FK_STGIMPORTINT_JOBS_idx` (`jobId`),
  KEY `FK_STGIMPORTINT_RELATIONSHIPS_idx` (`relationshipId`),
  KEY `FK_ISI_1` (`jobId`,`relationshipId`,`processingStatus`,`processingDateTime`,`isDeleted`),
  KEY `IDX_ISI_LOAD` (`jobId`,`relationshipId`,`processingStatus`,`isDeleted`,`transactionType`,`referenceText_1`),
  CONSTRAINT `FK_STGIMPORTINT_JOBIMPORTS` FOREIGN KEY (`jobImportId`) REFERENCES `etl_jobimports` (`jobimportid`),
  CONSTRAINT `FK_STGIMPORTINT_JOBS` FOREIGN KEY (`jobId`) REFERENCES `etl_jobs` (`jobid`),
  CONSTRAINT `FK_STGIMPORTINT_RELATIONSHIPS` FOREIGN KEY (`relationshipId`) REFERENCES `m_relationships` (`relationshipid`)
) ENGINE=InnoDB AUTO_INCREMENT=32768 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='RECO job stores the user imported erp data in this staging table. Copying to this table is done in a "Batch" and entire source data is stored as a single JSON. Import LogID is the unique ID generated for each import performed. ExecutionLogId is the unique ID generated for each execution of RECO job. Initially all records are marked as "New". After RECO job execution, if all records are matched then the status is udpated as "Completed" else "Pending"';
...
-- Dump completed on 2019-01-08 12:42:25
