...
SET vReturn = 0;
   ...
   -- DATA:  0999914BG0000974
	...
	update t_intrecords
	set derivedCol_11 = REGEXP_SUBSTR(referenceText_11, '[0-9]{14}[a-z]{2}[0-9]{11}')
	where referenceText_11 regexp '0999.*[0-9]{14}[a-z]{2}[0-9]{11}' 
    and jobId = vJobId
	and relationshipId = vrelationshipId
	and isDeleted='N';
...
SET vReturn = 0;
	...
	from 
	(
		select e.extRecordsId,  eq1.BankUniqCode1, eq1.BankUniqCode2, e.amount_1 as BankAmount 
		from t_extrecords e 
		inner join 
		(
	...
	inner join
	(
		select i.intRecordsId, iq1.ERPUniqCode1 , iq1.ERPUniqCode2, iq1.ERPAmount
		from t_intrecords i 
		inner join  
		(
			select referenceDateTime_1 as ERPUniqCode1, referenceText_11 as ERPUniqCode2, sum(amount_1) as ERPAmount 
			from t_intrecords 
			where 
				referenceText_11 is not null
				and jobId = vJobId
				and relationshipId = vRelationShipId
                and isDeleted = 'N'
				and processingStatus IN ('New','Open')
    ...
    inner join t_recoresults r 
		on i.intRecordsId = r.t_intRecordId
	set  
        ...
        i.lastUpdatedBy = vUserName ,
		i.lastUpdatedDt = vUpdateToken,
        i.transactionType = 'MATCHED-TEXT',
		i.transactionSubType = 'R10_1' 
	where	
		i.jobId = vJobId
		and i.relationshipId = vrelationshipId	
	...
	set  
		...
		e.transactionType = 'MATCHED-TEXT',
		e.transactionSubType = 'R10_1' 
	where	
		e.jobId = vJobId
		and e.relationshipId = vrelationshipId	
		and r.processedDt = vUpdateToken
...
SET vReturn = 0;
	...
	on 	 
		instr(q2.BankUniqCode2,q1.ERPUniqCode2) >0
	...
	IF iRowCount >0 then  
		...
		vupdateToken, vUserName, 'R10_G_AM_1'  , vJobExecutionId, vRelationshipId, GroupRank
		...
		set  
			...
			i.lastUpdatedBy = vUserName ,
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'GROUPED-TEXT',
            i.transactionSubType = 'R10_G_1_AM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
		...
		set  
			...
			e.lastUpdatedBy = vUserName ,
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'GROUPED-TEXT',
            e.transactionSubType = 'R10_G_1_AM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
...
SET vReturn = 0;
	...
	IF iRowCount >0 then  
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
			i.transactionType = 'GROUPED-TEXT',
            i.transactionSubType = 'R10_G_1_AM_2'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
            and processingStatus in ('New', 'Open') 
			and i.referenceText_4 is not null; 
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'GROUPED-TEXT',
            e.transactionSubType = 'R10_G_1_AM_2'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
...
SET vReturn = 0;
	...
	IF iRowCount >0 then  
		...
		set  
			...
			i.lastUpdatedBy = vUserName ,
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'GROUPED-FOREX',
            i.transactionSubType = 'R10_G_1_AM_3'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
		...
		set  
			...
			e.lastUpdatedBy = vUserName ,
			e.lastUpdatedDt = vUpdateToken,
			e.transactionType = 'GROUPED-FOREX',
            e.transactionSubType = 'R10_G_1_AM_3'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `P_0001_MR_10_G_AM_4` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`reco`@`localhost` PROCEDURE `P_0001_MR_10_G_AM_4`(  vOrgId INT,   vRelationshipId INT,   vRelationshipCode VARCHAR(16)  , vJobId INT,   vJobExecutionId INT  , vUserName VARCHAR(16), vTaskId INT, OUT vReturn INT )
BEGIN
...
SET vReturn = 0;
    ...
    -- special Case: "WITHDRAWAL TRANSFER"
	...
	drop temporary table if exists temp_group_10_g_4; 
    create temporary table temp_group_10_g_4
	...
	SET iRowCount = ROW_COUNT();
	SELECT '	P_0001_MR_10_G_AM_4 -> ROWS UPDATED:' , CONCAT(iRowCount), '	TIME:', NOW();
	INSERT INTO `reco`.`etl_recoexecutionlog`
	(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
    SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()) , 'TEMP 10_G_AM_4';
	...
	IF iRowCount >0 then    
        ...
        vupdateToken, vUserName, 'R10_G_AM_4'  , vJobExecutionId,vRelationshipId, groupRank
        from temp_group_10_g_4 t;
        
		SET iRowCount = ROW_COUNT();
		SELECT '	P_0001_MR_3G_1 -> ROWS UPDATED:' , CONCAT(iRowCount), '	TIME:', NOW();	 
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'RECO RESULTS 10_G_AM_4' ;

		-- Update the Group Id as well
		SELECT '	UPDATING PRIMARY SOURCE STATUS' ;        

		update t_intrecords i			
			inner join temp_group_10_g_4 g 
				on g.ERPUniqCode1 = i.referenceText_11  
		...
		set  
            ...
            i.transactionType = 'GROUPED-PATTERN',
            i.transactionSubType = 'R10_G_AM_4'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'PRIMARY 10_G_AM_4' ;
		...
		set  
			...
			e.transactionType = 'GROUPED-PATTERN',
            e.transactionSubType = 'R10_G_AM_4'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'SECONDARY 10_G_AM_4' ;



		-- set the group id
		select @iMaxRank:= max(GroupRank) from temp_group_10_g_4;       
		update recoSettings 
			set settingValue = @iMaxRank where settingKey='GroupSeqId';
	
		-- TODO : populate the groupRecords JSON with all ERP & BANK Primary ID's
		drop temporary table temp_group_10_g_4;
...
SET vReturn = 0;
	...
	IF iRowCount >0 then    
		...
		set  
            ...
            i.transactionSubType = 'R10_G_1_NM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
		...
		set  
			...
			e.transactionSubType = 'R10_G_1_NM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
   ...
   -- date variance
	...
	IF iRowCount >0 then    
		...
		set  
            ...
            i.transactionSubType = 'R10_G_1_NM_2'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
		...
		set  
			...
			e.transactionSubType = 'R10_G_1_NM_2'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
...
SET vReturn = 0;
	...
	from
    (
		select  intRecordsId, iq1.ERPUniqCode1, iq1.ERPUniqCode2, iq1.ERPAmount
		from t_intrecords i force index (IDX_ITR_1)
        inner join 
		(	SELECT  derivedCol_1 as ERPUniqCode1, referenceDateTime_1 as ERPUniqCode2,  sum(amount_1) as ERPAmount 
			FROM reco.t_intrecords 
			where 
				jobid=vJobId
				and relationshipid=vRelationshipId
			...
			group by derivedCol_1, referenceDateTime_1 
			having count(*) =1 
		) iq1 
        on 
			iq1.ERPUniqCode1 = i.derivedCol_1 
            and iq1.ERPUniqCode2 = i.referenceDateTime_1
            and iq1.ERPAmount = i.amount_1
            and i.jobId = vJobId
            and i.relationshipId= vRelationshipId
            and i.isDeleted='Y'
		where
	...
	(
		select extRecordsId, eq1.BankUniqCode1 , eq1.BankUniqCode2, eq1.BankAmount 
		from t_extrecords e FORCE INDEX (IDX_TER_1)
        inner join 
		(
			select derivedCol_1 as BankUniqCode1 , referenceDateTime_1 as BankUniqCode2 , sum(amount_1) as BankAmount
            from reco.t_extrecords 
			where 
		...
		on 
			eq1.BankUniqCode1 = e.derivedCol_1 
            and eq1.BankUniqCode2 = e.referenceDateTime_1
            and eq1.BankAmount = e.amount_1
			and e.jobId = vJobId
            and e.relationshipId= vRelationshipId
            and e.isDeleted='Y'
        where
	...
	set  
        ...
        i.lastUpdatedBy = vUserName ,
		i.lastUpdatedDt = vUpdateToken,
        i.transactionType = 'MATCHED-CHEQUES',
		i.transactionSubType = 'R1_1'
	where	
		i.jobId = vJobId
		and i.relationshipId = vRelationshipId	
		and r.processedDt = vUpdateToken
	...
	set  
        ...
        e.lastUpdatedBy = vUserName ,
		e.lastUpdatedDt = vUpdateToken,
		e.transactionType = 'MATCHED-CHEQUES',
		e.transactionSubType = 'R1_1'
	where	
		e.jobId = vJobId
		and e.relationshipId = vRelationshipId	
		and r.processedDt = vUpdateToken
...
SET vReturn = 0;
    ...
    SET vUpdateToken = now();
     ...
     -- 1: Find all the ERP Records which are cheque numbers (included in derivedCol_1). Refer BANK records and use derivedCol_1 column for Checque numbers. Store these combinations of ERP & BANK  in a Temp Table
	...
	from
	(		
			SELECT  derivedCol_1 as ERPUniqCode1, referenceDateTime_1 as ERPUniqCode2, 
			sum(case when abs(debitAmount) > 0 then debitAmount
				when abs(creditAmount) > 0 then creditAmount
			 end
			) as ERPAmount , count(*)  as ERPRowCount
			FROM reco.t_intrecords FORCE INDEX (IDX_ITR_1)
			where 
				jobid= vJobId              
				and relationshipid= vRelationshipid
				and processingStatus in ('New', 'Open') 
	...
	IF iRowCount >0 then  
       -- add to recoresults 1:Many matching.  
		...
		SELECT '	UPDATING PRIMARY SOURCE STATUS' ;
		update t_intrecords i
			inner  join temp_group_g_1 g 
            on g.ERPUniqCode1=i.derivedCol_1	
			and g.ERPUniqCode2 = i.referenceDateTime_1
		set  
			i.processingStatus = 'Processed' ,
			i.processingDateTime = vUpdateToken,
			...
			i.lastUpdatedBy = vUserName ,
			i.lastUpdatedDt = vUpdateToken,
			i.transactionType = 'GROUPED-CHEQUES',
            i.transactionSubType = 'R1_G_1_AM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vRelationShipId
            and i.isDeleted='Y'
            and i.derivedCol_1 is not null
            and i.processingStatus in ('New', 'Open') ;
		...
		update t_extRecords e
			inner  join temp_group_g_1 g 
            on g.BankUniqCode1 = e.derivedCol_1		
            and g.BankUniqCode2 = e.referenceDateTime_1
		set  
			e.processingStatus = 'Processed' ,
			e.processingDateTime = vUpdateToken,
			...
			e.lastUpdatedBy = vUserName ,
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'GROUPED-CHEQUES',
            e.transactionSubType = 'R1_G_1_AM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vRelationShipId
            and e.isDeleted='Y'
			and e.derivedCol_1 is not null
            and e.processingStatus in ('New', 'Open') ;
...
SET vReturn = 0;
    ...
    SET vUpdateToken = now();
     ...
     -- Grouped Records: 1:Many combinations (Bank:ERP). Nearest Match: Pattern Match (string)
     -- 1: Find all the ERP Records which are cheque numbers (included in referenceText_11 ). Refer BANK records and use derivedCol_1 column for Checque numbers. Store these combinations of ERP & BANK  in a Temp Table
	 -- 2: If records are found: Update the ERP & BANK Records with the GroupId's
     -- 3: Update the last generated GroupId in the recosettings table.
    ...
    select q1.ERPUniqCode1, q1.ERPUniqCode2, q1.ERPAmount, 
    q2.BankUniqCode1,  q2.BankUniqCode2, q2.BankAmount,
	Rank () Over (Order by  q1.ERPUniqCode1)  +  @iMyRank  as GroupRank
	from
	(		
	...
	IF iRowCount >0 then 		
		...
		set  
			...
			i.lastUpdatedBy = vUserName ,
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'PATTERN-MATCH-CHEQUE',
            i.transactionSubType = 'R1_G_1_NM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vRelationShipId
			and i.processingStatus in ('New', 'Open') 
		...
		set  
			...
			e.lastUpdatedBy = vUserName ,
			e.lastUpdatedDt = vUpdateToken,
			e.transactionType = 'PATTERN-MATCH-CHEQUE',
			e.transactionSubType = 'R1_G_1_NM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vRelationShipId
            and e.processingStatus in ('New', 'Open') 
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `P_0001_MR_1_G_NM_2` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`reco`@`localhost` PROCEDURE `P_0001_MR_1_G_NM_2`(  vOrgId INT,   vRelationshipId INT,   vRelationshipCode VARCHAR(16)  , vJobId INT,   vJobExecutionId INT  , vUserName VARCHAR(16), vTaskId INT, OUT vReturn INT )
BEGIN
...
SET vReturn = 0;
    ...
    SET vUpdateToken = now();
     ...
     -- Grouped Records: Many:Many combinations (Bank:ERP). Nearest Match: Amount MisMatch (Variance)
     -- 1: Find all the ERP Records which are cheque numbers (included in derivedCol_1 ). Refer BANK records and use derivedCol_1 column for Checque numbers. Store these combinations of ERP & BANK  in a Temp Table
	 -- 2: If records are found: Update the ERP & BANK Records with the GroupId's
     -- 3: Update the last generated GroupId in the recosettings table.
    ...
    drop temporary table if exists temp_group_g_2;    
	create temporary table temp_group_g_2   
	...
	INSERT INTO `reco`.`etl_recoexecutionlog`
	(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
	SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'TEMP R1_G_NM_2' ;
	...
	IF iRowCount >0 then 		
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'AMOUNT-VARIANCE',
            i.transactionSubType = 'R1_G_1_NM_2'
		where	
			i.jobId = vJobId
			and i.relationshipId = vRelationShipId
			and i.processingStatus in ('New', 'Open') 
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'PRIMARY R1_G_NM_2' ;
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
			e.transactionType = 'AMOUNT-VARIANCE',
			e.transactionSubType = 'R1_G_1_NM_2'
		where	
			e.jobId = vJobId
			and e.relationshipId = vRelationShipId
            and e.processingStatus in ('New', 'Open') 
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'SECONDARY R1_G_NM_2' ;

		select @iMaxRank:=max(GroupRank) from temp_group_g_2;		
		update recoSettings 
			set settingValue = @iMaxRank where settingKey='GroupSeqId';
    ...
    drop temporary table temp_group_g_2;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `P_0001_MR_1_G_NM_3` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`reco`@`localhost` PROCEDURE `P_0001_MR_1_G_NM_3`(  vOrgId INT,   vRelationshipId INT,   vRelationshipCode VARCHAR(16)  , vJobId INT,   vJobExecutionId INT  , vUserName VARCHAR(16), vTaskId INT, OUT vReturn INT )
BEGIN
...
SET vReturn = 0;
    ...
    SET vUpdateToken = now();
     ...
     -- Grouped Records: Many:Many combinations (Bank:ERP). Nearest Match: Date MisMatch (Date Variance)
     -- 1: Find all the ERP Records which are cheque numbers (included in derivedCol_1 ). Refer BANK records and use derivedCol_1 column for Checque numbers. Store these combinations of ERP & BANK  in a Temp Table
	 -- 2: If records are found: Update the ERP & BANK Records with the GroupId's
     -- 3: Update the last generated GroupId in the recosettings table.
    ...
    drop temporary table if exists temp_group_g_3;    
	create temporary table temp_group_g_3   
	...
	INSERT INTO `reco`.`etl_recoexecutionlog`
	(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
	SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'TEMP R1_G_NM_3' ;
	...
	IF iRowCount >0 then 		
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'DATE-VARIANCE',
            i.transactionSubType = 'R1_G_1_NM_3'
		where	
			i.jobId = vJobId
			and i.relationshipId = vRelationShipId
			and i.processingStatus in ('New', 'Open') 
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'PRIMARY R1_G_NM_3' ;
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
			e.transactionType = 'DATE-VARIANCE',
			e.transactionSubType = 'R1_G_1_NM_3'
		where	
			e.jobId = vJobId
			and e.relationshipId = vRelationShipId
            and e.processingStatus in ('New', 'Open') 
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'SECONDARY R1_G_NM_3' ;

		select @iMaxRank:=max(GroupRank) from temp_group_g_3;		
		update recoSettings 
			set settingValue = @iMaxRank where settingKey='GroupSeqId';
    ...
    drop temporary table temp_group_g_3;
...
SET vReturn = 0;
    ...
    (
        ...
        inner join 
		(	SELECT  derivedCol_2 as ERPUniqCode1, referenceDateTime_1 as ERPUniqCode2, sum(amount_1) as ERPAmount
			FROM reco.t_intrecords 
			where 
				jobid=vJobId
				and relationshipid=vrelationshipId
        ...
        on 
			iq1.ERPUniqCode1 = i.derivedCol_2 
            and iq1.ERPUniqCode2  = i.referenceDateTime_1
            and iq1.ERPAmount = i.amount_1
            and i.jobId = vJobId
            and i.relationshipId= vRelationshipId
	...
	(
        ...
        inner join 
		(
			select derivedCol_2  as BankUniqCode1, referenceDateTime_1 as BankUniqCode2, sum(amount_1) as BankAmount 
            from reco.t_extrecords 
			where 
		...
		on 
			eq1.BankUniqCode1 = e.derivedCol_2 			
			and eq1.BankUniqCode2 = e.referenceDateTime_1
            and eq1.BankAmount  = e.amount_1
            and e.jobId = vJobId
            and e.relationshipId= vRelationshipId
	...
	set  
        ...
        i.lastUpdatedBy = vUserName ,
		i.lastUpdatedDt = vupdateToken,   
		i.transactionType = 'MATCHED-UTR',
		i.transactionSubType = 'R2_1'
	where	
		i.jobId = vJobId
		and i.relationshipId = vrelationshipId	
		and r.processedDt = vupdateToken
	...
	set  
        ...
        e.lastUpdatedBy = vUserName ,
		e.lastUpdatedDt = vupdateToken,
		e.transactionType = 'MATCHED-UTR',
		e.transactionSubType = 'R2_1'
	where	
		e.jobId = vJobId
		and e.relationshipId = vrelationshipId	
		and r.processedDt = vupdateToken
...
SET vReturn = 0;
	...
	(
		select derivedCol_2 as BankUniqCode1, referenceDateTime_1 as BankUniqCode2,
        sum(case when abs(debitAmount) > 0 then debitAmount
			when abs(creditAmount) > 0 then creditAmount
		 end
		) as BankAmount,  count(*) 
	...
	IF iRowCount >0 then  
		...
		set  
			...
			i.lastUpdatedBy = vUserName ,
			i.lastUpdatedDt = vUpdateToken,
			i.transactionType = 'GROUPED-UTR',
            i.transactionSubType = 'R2_G_1_AM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
			and i.derivedCol_2 is not null
            and i.processingStatus in ('New', 'Open') ;    
		...
		set  
			...
			e.lastUpdatedBy = vUserName ,
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'GROUPED-UTR',
            e.transactionSubType = 'R2_G_1_AM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
...
SET vReturn = 0;
    ...
    SET vUpdateToken = now();
     ...
     -- Grouped Records: 1:Many combinations (Bank:ERP). Auto Match (Search ReferenceText_11 from Bank). Search ERP Text for the UTR number. 
...
SET vReturn = 0;
	...
	INSERT INTO `reco`.`etl_recoexecutionlog`
	(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
	SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'TEMP R2_G_NM_1' ;
	...
	IF iRowCount >0 then    	
		...
		set 
			...
			i.processingDateTime = vUpdateToken,
			i.jobExecutionId = vjobExecutionId,
			i.groupId = t.GroupRank,
			i.isMatched = 'N',
			i.lastUpdatedBy = vUserName ,
			i.lastUpdatedDt = vUpdateToken,
			i.transactionType = 'AMOUNT-VARIANCE',
            i.transactionSubType = 'R2_G_NM_1'
		where i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N';   
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'PRIMARY R2_G_NM_1' ;
		...
		set 
			...
			e.processingDateTime = vUpdateToken,
			e.jobExecutionId = vjobExecutionId,
			e.groupId = t.GroupRank,
			e.isMatched = 'N',
			e.lastUpdatedBy = vUserName ,
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'AMOUNT-VARIANCE',
            e.transactionSubType = 'R2_G_NM_2'
		where e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N';
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'SECONDARY R2_G_NM_1' ;
...
SET vReturn = 0;
	...
	IF iRowCount >0 then    	
		...
		set 
			...
			i.lastUpdatedDt = vUpdateToken,
			i.transactionType = 'DATE-VARIANCE',
            i.transactionSubType = 'R2_G_NM_2'
		where i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N';   
...
SET vReturn = 0;
    ...
    drop temporary table if exists temp_group_2_g_3;    
    create temporary table temp_group_2_g_3    
    select q1.ERPUniqCode1 ,  q1.ERPAmount, 
    q2.BankUniqCode1  , q2.BankAmount,
	Rank () Over (Order by  q1.ERPUniqCode1 )   + @iMyRank  as GroupRank
	from
	...
	on 	 
		...
		SET iRowCount = ROW_COUNT();
		SELECT '	P_0001_MR_2_G_NM_3 -> ROWS UPDATED:' , CONCAT(iRowCount), '	TIME:', NOW();	 
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
	...
	IF iRowCount>0 then	
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
			i.transactionType = 'AMOUNT-VARIANCE',
            i.transactionSubType = 'R2_G_NM_3'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N';      
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'AMOUNT-VARIANCE',
            e.transactionSubType = 'R2_G_NM_3'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N';
		...
		select @iMaxRank:=max(GroupRank) from temp_group_2_g_3;
		update recoSettings 
			set settingValue = @iMaxRank where settingKey='GroupSeqId';
		
        -- TODO: Update groupedRecords JSON   
    
        drop temporary table temp_group_2_g_3;
...
SET vReturn = 0;
	...
	START TRANSACTION;
    SET vupdateToken = now();
	-- Vendor Name matching 1:1 (uses Soundex)
	 INSERT INTO `reco`.`t_recoresults`
	( 
		`t_intRecordId`,
		`t_extRecordId`,
	...
	from
    (
		select  intRecordsId, iq1.ERPUniqcode1, iq1.ERPUniqcode2, iq1.ERPAmount, iq1.Soundex1
		from t_intrecords i  FORCE INDEX (IDX_ITR_3)
        inner join 
		(	SELECT  derivedCol_3 as ERPUniqcode1 , referenceDateTime_1 as ERPUniqcode2, sum(amount_1) as ERPAmount, soundex(derivedCol_3) as Soundex1
			FROM reco.t_intrecords FORCE INDEX (IDX_ITR_3)
			where 
				jobid=vJobId
				and relationshipid=vrelationshipId
        ...
        on 
			iq1.ERPUniqcode1 = i.derivedCol_3
            and iq1.ERPUniqcode2 = i.referenceDateTime_1
			and iq1.ERPAmount = i.amount_1   
            and i.jobId = vJobId
            and i.relationshipId= vRelationshipId
	...
	(
		select extRecordsId, eq1.BankUniqcode1,  eq1.BankUniqcode2, eq1.BankAmount, eq1.Soundex1
		from t_extrecords e FORCE INDEX (IDX_TER_3)
        inner join 
		(
			select derivedCol_3 as BankUniqcode1 ,referenceDateTime_1 as BankUniqcode2,  sum(amount_1) as BankAmount, soundex(derivedCol_3) as Soundex1
            from reco.t_extrecords FORCE INDEX (IDX_TER_3)
			where 
		...
		on 
			eq1.BankUniqcode1 = e.derivedCol_3 
            and eq1.BankUniqcode2 = e.referenceDateTime_1
			and eq1.BankAmount = e.amount_1
            and e.jobId = vJobId
            and e.relationshipId= vRelationshipId
    ...
    on 
		( 
			instr(q1.Soundex1, q2.Soundex1) >0 
			or instr(q2.Soundex1, q1.Soundex1) >0
            or q1.ERPUniqcode1 =  q2.BankUniqcode1
            )
		and q1.ERPUniqcode2 = q2.BankUniqcode2
		and q1.ERPAmount=q2.BankAmount;
	...
	IF iRowCount >0 THEN
		...
		set  
			...
			i.lastUpdatedBy = vUserName ,
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'MATCHED-VENDORS',
			i.transactionSubType = 'R3_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and r.processedDt = vupdateToken
		...
		set  
			...
			e.lastUpdatedBy = vUserName ,
			e.lastUpdatedDt = vUpdateToken ,
            e.transactionType = 'MATCHED-VENDORS',
			e.transactionSubType = 'R3_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId	
			and r.processedDt = vupdateToken
...
SET vReturn = 0;
    ...
    SET vUpdateToken = now();
     ...
     -- Grouped Records: Handles Many:Many combinations (ERP:Bank). Auto Matching. Combination of (derivedCol_3 & ReferenceDateTime, Amount) and Text Pattern Matching using Soundex or direct match.
     -- 1: Populate a temp table with all  (1:Many) (Many:1) (Many:Many) Combinations
     -- 2: Update RecoResults table with 1:1 combination. Update ERP & Bank Tables respectively.
	...
	(		
		select derivedCol_3 as BankUniqcode1, referenceDateTime_1 BankUniqcode2, soundex(derivedCol_3) as Soundex1,
		sum(
			case when abs(debitAmount) > 0 then debitAmount
				when abs(creditAmount) > 0 then creditAmount
			end
	...
	INSERT INTO `reco`.`etl_recoexecutionlog`
	(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
    SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()) , 'TEMP R3_G_AM_1';
	...
	IF iRowCount >0 then    
		...
		select null, null,  'MATCHED', 'SYSTEM-MATCHED', NULL, 'Y' , 
        vupdateToken, vUserName, 'R3_G_AM_1'  , vJobExecutionId,vRelationshipId, groupRank
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'RECO RESULTS R3_G_AM_1' ;
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
            i.groupId = g.groupRank,
            i.transactionType = 'GROUPED-VENDORS',
            i.transactionSubType = 'R3_G_AM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'PRIMARY R3_G_AM_1' ;
		...
		set  
            ...
            e.groupId = g.groupRank,
			e.transactionType = 'GROUPED-UTR',
            e.transactionSubType = 'R3_G_AM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'SECONDARY R3_G_AM_1' ;
...
SET vReturn = 0;
	...
	IF iRowCount >0 then    
		...
		set  
            ...
            i.groupId = g.groupRank,
			i.transactionType = 'GROUPED-VENDORS',
            i.transactionSubType = 'R3_G_AM_2'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
		...
		set  
            ...
            e.groupId = g.groupRank,
			e.transactionType = 'GROUPED-UTR',
            e.transactionSubType = 'R3_G_AM_2'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
...
SET vReturn = 0;
	...
	IF iRowCount >0 then    
		...
		set  
            ...
            i.groupId = g.groupRank,
            i.transactionType = 'AMOUNT-VARIANCE',
            i.transactionSubType = 'R3_G_NM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
		...
		set  
            ...
            e.groupId = g.groupRank,
			e.transactionType = 'AMOUNT-VARIANCE',
            e.transactionSubType = 'R3_G_NM_2'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
...
SET vReturn = 0;
	...
	IF iRowCount >0 then    
		...
		set  
            ...
            i.groupId = g.groupRank,
			i.transactionType = 'DATE-VARIANCE',
            i.transactionSubType = 'R3_G_NM_2'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
...
SET vReturn = 0;
	...
	from 
	(
		select e.extRecordsId,  eq1.BankUniqCode1, eq1.BankUniqCode2,  eq1.BankUniqCode3, eq1.BankAmount 
		from t_extrecords e 
		inner join 
		(
			select derivedCol_4 as BankUniqCode1, derivedCol_11 as BankUniqCode2, referenceDateTime_1 as BankUniqCode3, sum(amount_1) as BankAmount
			from t_extrecords 
			where   
				...
				and relationshipId = vRelationShipId
                and isDeleted = 'N'
			group by derivedCol_4 , derivedCol_11 , referenceDateTime_1
			having count(*) = 1
		) eq1 on (eq1.BankUniqCode1) = (e.derivedCol_4) 
			and (eq1.BankUniqCode2) = (e.derivedCol_11)	
            and (eq1.BankUniqCode3) = (e.referenceDateTime_1)	
			and e.jobId = vJobId
			and e.relationshipId= vRelationshipId
	) q1 
	inner join
	(
		select i.intRecordsId, iq1.ERPUniqCode1 , iq1.ERPUniqCode2,  iq1.ERPUniqCode3, iq1.ERPAmount
		from t_intrecords i 
		inner join  
		(
			select derivedCol_4 as ERPUniqCode1, derivedCol_11 as ERPUniqCode2 , referenceDateTime_1 as ERPUniqCode3, sum(amount_1) as ERPAmount 
			from t_intrecords 
            ...
            and (iq1.ERPUniqCode3) = (i.referenceDateTime_1)	
			and i.jobId = vJobId
			and i.relationshipId= vRelationshipId
	) q2 
	on  
        ...
        and (q1.BankUniqCode3) = (q2.ERPUniqCode3)
		and q1.BankAmount = q2.ERPAmount;
	...
	set  
        ...
        i.lastUpdatedBy = vUserName ,
		i.lastUpdatedDt = vUpdateToken,
        i.transactionType = 'MATCHED-REFCODE',
		i.transactionSubType = 'R4_1'
	where	
		i.jobId = vJobId
		and i.relationshipId = vrelationshipId	
		and r.processedDt = vUpdateToken
	...
	set  
        ...
        e.lastUpdatedBy = vUserName ,
		e.lastUpdatedDt = vUpdateToken,
        e.transactionType = 'MATCHED-REFCODES',
		e.transactionSubType = 'R4_1'
	where	
		e.jobId = vJobId
		and e.relationshipId = vrelationshipId	
		and r.processedDt = vUpdateToken
...
SET vReturn = 0;
    ...
    -- use combination of (derivedCol_4, referenceDateTime_1, Amount) and where derivedCol_11 is null 
    ...
    (
        ...
        inner join 
		(	SELECT  derivedCol_4 as ERPUniqCode1 ,  referenceDateTime_1 as ERPUniqCode2, sum(amount_1) as ERPAmount
			FROM reco.t_intrecords 
			where 
				jobid=vJobId
                and derivedCol_4 is not null 
	...
	(
        ...
        inner join 
		(
			select derivedCol_4 as BankUniqCode1,  referenceDateTime_1 as BankUniqCode2, sum(amount_1) as BankAmount
            from reco.t_extrecords 
			where 
				derivedCol_4 is not null 
                and derivedCol_11 is null 
...
SET vReturn = 0;
    ...
    SET vUpdateToken = now();
     ...
     -- Grouped Records: Handles Many:Many combinations (ERP:Bank). 
     -- Auto Matching. Combination of (derivedCol_4, derivedCol_11, ReferenceDateTime)
     -- 1: Populate a temp table with all  (1:Many) (Many:1) (Many:Many) Combinations
     -- 2: Update RecoResults table with 1:1 combination. Update ERP & Bank Tables respectively.
    ...
    select q1.ERPUniqCode1, q1.ERPUniqCode2, q1.ERPUniqCode3, q1.ERPAmount, q1.RowCount as ERPRowCount,
    q2.BankUniqCode1, q2.BankUniqCode2, q2.BankAmount, q2.BankUniqCode3, q2.RowCount as BankRowCount,
    Rank () Over (Order by  q1.ERPUniqCode1,  q1.ERPUniqCode2, q1.ERPUniqCode3) + @iMyRank as groupRank
	from
    (
		SELECT  derivedCol_4 as ERPUniqcode1 , derivedCol_11 as ERPUniqCode2, referenceDateTime_1 as ERPUniqCode3,
		sum(
			case when abs(debitAmount) > 0 then debitAmount
				when abs(creditAmount) > 0 then creditAmount
			end
    ...
    inner join 
	(		
		select derivedCol_4 as BankUniqcode1, derivedCol_11 BankUniqcode2, referenceDateTime_1 as BankUniqCode3,
		sum(
			case when abs(debitAmount) > 0 then debitAmount
				when abs(creditAmount) > 0 then creditAmount
			end
    ...
    on 
		q1.ERPUniqcode1 = q2.BankUniqcode1
		and q1.ERPUniqcode2 = q2.BankUniqcode2
        and q1.ERPUniqcode3 = q2.BankUniqcode3
		and q1.ERPAmount = q2.BankAmount ;
	...
	IF iRowCount >0 then    
		...
		update t_intrecords i			
			inner join temp_group_4_g_1 g 
                ...
                and i.referenceDateTime_1  = g.ERPUniqCode3
		set  
			i.processingStatus = 'Processed' ,
			i.processingDateTime = vUpdateToken,
			i.jobExecutionId = vjobExecutionId,		
			...
			i.lastUpdatedDt = vUpdateToken,
            i.groupId = g.groupRank,
			i.transactionType = 'GROUPED-REFCODES',
            i.transactionSubType = 'R4_G_AM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
            and i.derivedCol_4 is not null
			and i.derivedCol_11 is not null
            and i.processingStatus in ('New', 'Open') ;
		...
		update t_extrecords e		
			inner join temp_group_4_g_1 g 
                ...
                and e.referenceDateTime_1  = g.BankUniqCode3 
		set  
			e.processingStatus = 'Processed' ,
			e.processingDateTime = vUpdateToken,
			e.jobExecutionId = vjobExecutionId,		
			...
			e.lastUpdatedDt = vUpdateToken,
            e.groupId = g.groupRank,
			e.transactionType = 'GROUPED-REFCODES',
            e.transactionSubType = 'R4_G_AM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
            and e.derivedCol_4 is not null
            and e.derivedCol_11 is not null
            and e.processingStatus in ('New', 'Open') ;
...
SET vReturn = 0;
	...
	IF iRowCount >0 then  
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'GROUPED-REFCODES',
            i.transactionSubType = 'R4_G_AM_2'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'GROUPED-REFCODES',
            e.transactionSubType = 'R4_G_AM_2'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `P_0001_MR_4_G_AM_3` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`reco`@`localhost` PROCEDURE `P_0001_MR_4_G_AM_3`(  vOrgId INT,   vRelationshipId INT,   vRelationshipCode VARCHAR(16)  , vJobId INT,   vJobExecutionId INT  , vUserName VARCHAR(16), vTaskId INT, OUT vReturn INT )
BEGIN
...
SET vReturn = 0;
    ...
    drop temporary table if exists temp_group_4_g_3;    
    create temporary table temp_group_4_g_3    
    select q1.ERPUniqCode1,  ERPUniqCode2, q1.ERPAmount, 
    q2.BankUniqCode1, q2.BankUniqCode2, q2.BankAmount,
	Rank () Over (Order by  q1.ERPUniqCode1, q1.ERPUniqCode2)   + @iMyRank  as GroupRank
	from
	...
	SET iRowCount = ROW_COUNT();
	SELECT '	P_0001_MR_4_G_AM_3 -> ROWS UPDATED:' , CONCAT(iRowCount), '	TIME:', NOW();	 
	INSERT INTO `reco`.`etl_recoexecutionlog`
	(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
	SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'TEMP R4G_AM_3' ;
	...
	IF iRowCount >0 then  
		...
		select  null, null, 'MATCHED', 'SYSTEM-MATCHED', 'GROUPED', 'Y' , 
		vupdateToken, vUserName, 'R4_G_AM_3'  , vJobExecutionId, vRelationshipId, GroupRank
		from temp_group_4_g_3;
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'RECORESULT R4_G_AM_3' ;

		-- Update the Group Id as well
		SELECT '	UPDATING PRIMARY SOURCE STATUS' ;
		update t_intrecords i
		inner join temp_group_4_g_3 g
			on g.ERPUniqCode1 = i.derivedCol_4
            and g.ERPUniqCode2= i.referenceDateTime_1
		set  
			i.processingStatus = 'Processed' ,
			...
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'GROUPED-REFCODES',
            i.transactionSubType = 'R4_G_AM_3'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'PRIMARY R4_G_AM_3' ;
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
            e.transactionType = 'GROUPED-REFCODES',
            e.transactionSubType = 'R4_G_AM_3'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
		...
		INSERT INTO `reco`.`etl_recoexecutionlog`
		(`jobId`,`executionId`,`taskId`,`startDt`,`endDt`,`status`,`rowCount`,`duration`, `comments`)   
		SELECT vJobId, vJobExecutionId, vTaskid, vUpdateToken, now(), 'SUCCESS' , iRowCount, TIMESTAMPDIFF(SECOND , vUpdateToken,  now()), 'SECONDARY R4_G_AM_3' ;

		select @iMaxRank:=max(GroupRank) from temp_group_4_g_3;
		update recoSettings 
			set settingValue = @iMaxRank where settingKey='GroupSeqId';
		
        -- TODO: Update groupedRecords JSON   
    
        drop temporary table temp_group_4_g_3;
...
SET vReturn = 0;
   ...
   -- Amount Variance. (similar to 4_G_AM_1)
	...
	IF iRowCount >0 then    
		...
		set  
            ...
            i.groupId = g.groupRank,
			i.transactionType = 'AMOUNT-VARIANCE',
            i.transactionSubType = 'R4_G_NM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
		...
		set  
            ...
            e.groupId = g.groupRank,
            e.transactionType = 'AMOUNT-VARIANCE',
            e.transactionSubType = 'R4_G_NM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
...
SET vReturn = 0;
	...
	-- Amount Variance (similar to 4_G_AM_2)
    select @iMyRank:=settingValue from recoSettings where  settingKey='GroupSeqId'; 
	...
	IF iRowCount >0 then    
		...
		set  
            ...
            i.groupId = g.groupRank,
			i.transactionType = 'AMOUNT-VARIANCE',
            i.transactionSubType = 'R4_G_NM_2'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId
            and i.isDeleted='N'
            and i.derivedCol_4 is not null
            and i.derivedCol_11 is null
            and i.processingStatus in ('New', 'Open') ;
		...
		set  
            ...
            e.groupId = g.groupRank,
			e.transactionType = 'AMOUNT-VARIANCE',
            e.transactionSubType = 'R4_G_NM_2'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
            and e.isDeleted='N'
...
SET vReturn = 0;
    ...
    (
        ...
        inner join 
		(	SELECT  derivedCol_5 as ERPUniqCode1,  referenceDateTime_1 as ERPUniqCode2, sum(amount_1) as ERPAmount, count(*) 
			FROM reco.t_intrecords 
			where 
				jobid=vJobId
				and relationshipid=vrelationshipId
				and processingStatus in ('New', 'Open') 
				and isDeleted='N'				 
			group by derivedCol_5, referenceDateTime_1
			having count(*) =1 
		) iq1 
        on 
			iq1.ERPUniqCode1 = i.derivedCol_5	
            and iq1.ERPUniqCode2 =i.referenceDateTime_1
            and i.jobId = vJobId
            and i.relationshipId= vRelationshipId
            and i.derivedCol_5 is not null 
		where
			i.processingStatus in ('New', 'Open') 
			and i.isDeleted='N'			
	) q1 inner join 
	(
        ...
        inner join 
		(
			select derivedCol_5 as  BankUniqCode1, referenceDateTime_1 as BankUniqCode2, sum(amount_1) as BankAmount, count(*) from reco.t_extrecords 
			where 
		...
		on 
			eq1.BankUniqCode1 = e.derivedCol_5 
            and eq1.BankUniqCode2 = e.referenceDateTime_1
            and e.jobId = vJobId
            and e.relationshipId= vRelationshipId
        where
	...
	set  
        ...
        e.lastUpdatedBy = vUserName ,
		e.lastUpdatedDt = vUpdateToken,
		e.transactionType = 'MATCHED-REFCODE',
		e.transactionSubType = 'R5_1'
	where	
		e.jobId = vJobId
		and e.relationshipId = vrelationshipId	
		and r.processedDt = vupdateToken
...
SET vReturn = 0;
	...
	IF iRowCount >0 then  
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'GROUPED-REFCODES',
            i.transactionSubType = 'R5_G_AM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
			e.transactionType = 'GROUPED-REFCODES',
            e.transactionSubType = 'R5_G_AM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
			and e.derivedCol_5 is not null
            and e.processingStatus in ('New', 'Open') ;
...
SET vReturn = 0;
	...
	IF iRowCount >0 then  
		...
		set  
			...
			i.lastUpdatedDt = vUpdateToken,
            i.transactionType = 'AMOUNT-VARIANCE',
            i.transactionSubType = 'R5_G_NM_1'
		where	
			i.jobId = vJobId
			and i.relationshipId = vrelationshipId	
			and i.isDeleted='N'
		...
		set  
			...
			e.lastUpdatedDt = vUpdateToken,
			e.transactionType = 'AMOUNT-VARIANCE',
            e.transactionSubType = 'R5_G_NM_1'
		where	
			e.jobId = vJobId
			and e.relationshipId = vrelationshipId
			and e.isDeleted='N'
...
SET vReturn = 0;
    ...
    (
		select  intRecordsId, iq1.ERPUniqCode1, iq1.ERPUniqCode2, iq1.ERPAmount
		from t_intrecords i force index (IDX_ITR_7)
        inner join 
		(	SELECT  derivedCol_7 as  ERPUniqCode1, referenceDateTime_1 as ERPUniqCode2, sum(amount_1) as ERPAmount, count(*) 
			FROM reco.t_intrecords 
			where 
				jobid=vJobId
				and relationshipid=vrelationshipId
			...
			group by derivedCol_7, referenceDateTime_1  
			having count(*) =1 
		) iq1 
        on 
			iq1.ERPUniqCode1 = i.derivedCol_7	
            and iq1.ERPUniqCode2 = i.referenceDateTime_1
            and i.jobId = vJobId
            and i.relationshipId= vRelationshipId
		where
	...
	(
		select e.extRecordsId, eq1.BankUniqCode1, eq1.BankUniqCode2, eq1.BankAmount
		from t_extrecords e FORCE INDEX (IDX_TER_7)
        inner join 
		(
			select derivedCol_7 as  BankUniqCode1, referenceDateTime_1 as BankUniqCode2, sum(amount_1) as BankAmount, count(*)  
            from reco.t_extrecords 
			where 
		...
		on 
			eq1.BankUniqCode1 = e.derivedCol_7	
            and eq1.BankUniqCode2 = e.referenceDateTime_1
            and e.jobId = vJobId
            and e.relationshipId= vRelationshipId
        where
